...
class CardwireBuilderPanelTest : BasePlatformTestCase() {

    private fun CardwireBuilderPanel.set(field: Int, value: String) {
        val row = fieldsModel.rows.indexOfFirst { it.spec.id == field }
        fieldsModel.setValueAt(value, row, CardwireBuilderPanel.FieldsTableModel.VALUE)
    }

    fun testBuildsHexFromTheForm() {
        val panel = CardwireBuilderPanel()
