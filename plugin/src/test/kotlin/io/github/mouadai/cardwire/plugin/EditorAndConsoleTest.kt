...
class EditorAndConsoleTest : BasePlatformTestCase() {
    ...
    fun testDecodeActionsAreRegisteredInEditorAndConsoleMenus() {
        ...
        assertTrue(actions.getAction("Cardwire.DecodeSelectionAsIso") is DecodeSelectionAsIsoAction)
        assertTrue(actions.getAction("Cardwire.DecodeSelectionAsEmv") is DecodeSelectionAsEmvAction)
        assertNotNull(actions.getAction("Cardwire.DecodeSelection"))
    }

    fun testConsoleFilterIsOffByDefault() {
        assertFalse(CardwireSettings.getInstance().state.consoleFilterEnabled)
        val filter = CardwireConsoleFilterProvider().getDefaultFilters(project).single()
