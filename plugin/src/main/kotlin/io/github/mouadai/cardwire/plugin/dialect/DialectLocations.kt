...
object DialectLocations {
    ...
    const val PROJECT_DIR = ".cardwire/dialects"

    /** Bundled JSON Schema for dialect files, shipped in `core`. */
    const val SCHEMA_RESOURCE = "/cardwire/dialect/dialect.schema.json"

    /** Per-user dialect folder in the IDE config directory. */
    fun userDir(): Path = PathManager.getConfigDir().resolve("cardwire").resolve("dialects")
    ...
    fun affectsDialects(path: String): Boolean {
        ...
        // Creating, deleting or renaming the folder itself (or its .cardwire parent).
        return p.endsWith("/.cardwire") || p.endsWith("/$PROJECT_DIR") || isDialectDirPath(p)
    }
}
