...
import io.github.mouadai.cardwire.core.jpos.JposImportResult
import io.github.mouadai.cardwire.core.jpos.JposPackagerImporter
...
class ImportJposPackagerAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (!CardwireLicense.isEnabled(PaidFeature.JPOS_IMPORT)) {
            CardwireLicense.requestLicense(PaidFeature.JPOS_IMPORT)
            return
        }
    ...
    companion object {
        const val ID = "Cardwire.ImportJposPackager"
...
object JposDialectImport {
    const val DIALECT_DIR = ".cardwire/dialects"

    class Outcome(val result: JposImportResult, val file: VirtualFile?, val writeError: String?)

    /** Converts [xml] and writes it to `.cardwire/dialects/<id>.json` in the project, never overwriting a file. */
