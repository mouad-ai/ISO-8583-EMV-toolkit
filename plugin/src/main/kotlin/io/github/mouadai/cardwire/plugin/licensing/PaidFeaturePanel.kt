...
class PaidFeaturePanel(
    ...
    private val isEnabled: () -> Boolean = { CardwireLicense.isEnabled(feature) },
    ...
    private fun lockedView(): JComponent = panel {
        row { label("${feature.label} is part of Cardwire Pro.") }
        row {
            button("Enter License...") { CardwireLicense.requestLicense(feature) }
            button("Check Again") {
                CardwireLicense.invalidate()
                refresh()
            }
        }
    }
