...
/**
 * The only place Octet asks about its license. Octet is freemium: the `product-descriptor` in
 * plugin.xml is optional, so the plugin always loads and only [PaidFeature]s check [isEnabled].
 */
object OctetLicense {

    /**
     * Must equal the `code` of `product-descriptor` in plugin.xml. Placeholder until JetBrains
     * Marketplace assigns the real code to the approved paid plugin.
     */
    const val PRODUCT_CODE = "POCTET"
     ...
     */
    const val DEV_PROPERTY = "octet.license.dev"
    ...
    /** Opens the IDE's registration dialog with Octet preselected. */
    fun requestLicense(feature: PaidFeature) {
        val message = "${feature.label} is part of Octet Pro."
